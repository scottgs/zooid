...
var zode = merge( require("./package.json"),{
    takes:"terrain_silhouette"
  , gives:"terrain_signature"
  , ip:zooid.ip||'unknown'
  , status:"?"
  , work:0
  , actions:0 
...
zooid.muster(zode)
console.log(zode.name, "initiated")


zooid.on("terrain_silhouette", function(signal){

  getSignatureFromPoints(signal.terrain_silhouette, function(err, signature){
     zooid.send({
        name:"Terrain Signature"
      , noun:"terrain_signature"
      , parent_id:signal.id
...
zooid.on("test", function(test_signal){
  ...
  getSignatureFromPoints( signal.points, function(err, terrain_signature){
    
    if(err) { 
      zooid.send({ name:zode.name+" Failed Test"})
      zode.status = "error"
    } else {
      zode.status = "active"
    }
