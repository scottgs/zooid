...
ScissorsWorker.prototype.thresholdFilter = function(pixels) {
  var d = pixels;
  var threshold = 0.3
...
ScissorsWorker.prototype.computeGreyscale = function(pixels) {
	...
	var greyscale = new Float32Array(pixels.length / 4);

	for (var i = 0; i < pixels.length; i += 4) {
		greyscale[i/4] = (pixels[i] + pixels[i+1] + pixels[i+2]) / (3*255);
	}
...
ScissorsWorker.prototype.convertCanvasToImage = function (canvas) {
...
ScissorsWorker.prototype.setImageData = function(image, aoi, mask) {
	...
	// $('body').append(this.convertCanvasToImage(this.thresholdFilter(image)) );
