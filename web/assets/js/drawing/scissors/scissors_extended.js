...
// <added>
// By Kirk
//Dynamic path used for including extra files
var __scissors_scripts = document.getElementsByTagName("script");
var __scissors_path = __scissors_scripts[__scissors_scripts.length-1].src;
var __scissors_dir = __scissors_path.substring(0, __scissors_path.lastIndexOf('/'));
//</added>

// Makes a vector
Scissors.prototype.makeVector = function(path) {
   
   if( typeof squiggles === 'undefined'){
      squiggles = new Squiggles(path);
   } else {
      squiggles.addPath(path);
   }
    squiggles.appendToResultChart(path)
...
function Scissors() {
   this.lineColor = "black";//new Array(255, 0, 0, 255);
...
Scissors.prototype.init = function(img, mask, visualize) {
   ...
   this.output = document.getElementById("scissors_output");
   ...
   this.scissorsWorker = new ScissorsWorker(__scissors_dir+"/scissorsWorker.js");
...
Scissors.prototype.onError = function(event){
   if (this.output != null) this.output.textContent = event.message;
   throw new Error(event.message + " (" + event.filename + ":" + event.lineno + ")");
};

Scissors.prototype.onStatus = function(msg) {
   if (this.output != null) this.output.textContent = msg;
...
Scissors.prototype.updateDragScrolling = function(event) {
   ...
   var scrc = document.getElementById('scroll_content');
   scrc.scrollLeft = scrc.scrollLeft + tx*speed;
   scrc.scrollTop = scrc.scrollTop + ty*speed;
...
Scissors.prototype.submitScissors = function() {
   ...
   pathInput.setAttribute('value', JSON.stringify(this.paths));
...
// Attempts to encode the current paths array and add it to the scissors_form
// form object.
Scissors.prototype.submitScissors2 = function() {
   if ( this.requiresClosed() && !this.isClosed() ) {
      window.alert("Outline must form a complete loop, which it currently doesn't.");
      return false; // Cancel submission
   }
